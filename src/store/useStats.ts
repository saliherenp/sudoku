...
const EMPTY_DIFF: DiffStats = { played: 0, won: 0, bestTime: null, totalTime: 0 };
...
export function useStats() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(KEY).then(raw => {
      if (raw) {
        try { setStats({ ...DEFAULT, ...JSON.parse(raw) }); } catch {}
      }
    });
  }, []);

  const recordGame = async (difficulty: Difficulty, won: boolean, seconds: number) => {
    setStats(prev => {
